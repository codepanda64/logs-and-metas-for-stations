...
class Command(BaseCommand):
    def add_arguments(self, parser):
        parser.add_argument(
            "instrument_entities_file",
            type=str,
            help="The file that contains the seismic instrument entities info",
        )
        ...
        with open(f"{instrument_entities_file}.txt") as file:
            for line in file:
                ...
                InstrumentCategory.objects.get_or_create(
                    name=category, parent=category_parent
                )
                cat = InstrumentCategory.objects.get(name=category)

                Instrument.objects.get_or_create(
                    name=model, param=param, category=cat, is_model=True
                )
                instrument_model = Instrument.objects.get(name=model, param=param)
                ...
                entity, created = InstrumentEntity.objects.update_or_create(
                    sn=sn, defaults=defaults
                )
